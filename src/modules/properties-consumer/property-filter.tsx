...
const PropertyFilter = () => {
    ...
    const [localSearch, setLocalSearch] = useState<any>(
        JSON.parse(localStorage.getItem("property_filter") || "{}"),
    );
    ...
    const onDeletefilter = (filterKey: any, value: any) => {
        ...
        localStorage.setItem("property_filter", JSON.stringify(filters));
        setLocalSearch(filters);
    };

    const saveSearch = () => {
        //MIXPANEL : Event tracking for add new property requests
        mixpanel.track(`PROPERTY DASHBOARD : New Property Filter Saved `, {
            eventId: "new_property_filter_saved",
            ...getUserDetails(),
            ...getUserOrgDetails(),
            saved_filter: JSON.stringify(localSearch),
        });
        localStorage.setItem("property_filter", JSON.stringify(localSearch));
        enqueueSnackbar("", {
            variant: "success",
    ...
    const removeAllSearch = () => {
        localStorage.removeItem("property_filter");
        setLocalSearch({});
    };
