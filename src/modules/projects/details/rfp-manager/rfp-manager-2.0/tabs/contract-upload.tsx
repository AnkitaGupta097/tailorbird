...
export const isGoogleDocumentLink = (link: any) => {
    // Example of a Google Spreadsheet URL pattern: https://docs.google.com/document/d/SPREADSHEET_ID/edit
    const spreadsheetPattern = /https:\/\/docs\.google\.com\/document\/d\/([a-zA-Z0-9-_]+)/;
    return spreadsheetPattern.test(link);
};
const ContractUpload = () => {
    const dispatch = useAppDispatch();
    const { projectId } = useParams();
    const { contracts, allUsers, contractsLoading } = useAppSelector((state) => {
        return {
            contractorWithUsers: projectId,
            allUsers: state.tpsm.all_User?.users || [],
            contracts: state.singleProject.RFP.contracts.data,
            contractsLoading: state.singleProject.RFP.contracts.loading,
        };
    });
    ...
    const handleUpload = () => {
        ...
        if (!isGoogleDocumentLink(sheetLink)) {
            invalidLinkAlert();
        } else {
    ...
    const invalidLinkAlert = () => {
        enqueueSnackbar("", {
            variant: "error",
            action: (
                <BaseSnackbar variant={"error"} title={"Please provide valid google doc link."} />
            ),
        });
    };
    ...
    return (
        <div>
            ...
            >
                ...
                >
                    Upload Exhibit A contract
                </Typography>
                <Stack style={{ margin: "1.5rem 0 0.3rem 1.5rem" }}>
                    <FileUpload
                        uploaderId={"contract-upload"}
                        isMultiple={true}
                        accept=".pdf"
                        onFileChange={(file: any) => uploadToS3(file, false)}
                        helperText="Accepts .pdf and .doc only"
                        sizeLimitText="Maximum size: .pdf - 100 KB"
                        showsizeLimit={true}
                    />
                </Stack>
                <Typography
                ...
                >
                    You may also provide a Google Docs link as another option.
                </Typography>
                ...
                {contractsLoading && <LinearProgress />}
                ...
                <Accordion>
                    ...
                    >
                        <Typography variant="text_18_semibold" color={"#232323"}>
                            Previously Uploaded Contracts
                        </Typography>
                        <Divider />
                    </AccordionSummary>
                    <AccordionDetails>
                        {contracts?.length > 0 ? (
                            contracts?.map((uploadedItem: IFileDetails) => {
                                return (
